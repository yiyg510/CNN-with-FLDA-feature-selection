...
function Class_Map = Salinas_lda_wrapper(params) 

load Salinas_corrected.mat;
load Salinas_gt.mat;

disp( 'Data is imported...' );
GroundTruth=double(salinas_gt);
Salinas=single(salinas_corrected);

[ nRow, nColumn, nBand ] = size( Salinas );

HalfWidth=13;
Salinas = 1 * ( ( Salinas - min( Salinas( : ) ) ) / ( max( Salinas( : ) ) - min( Salinas( : ) ) )-0.5);
...
featuremap1=[];
for i=1:1615 
    featuremap1(i,:)=salinas_corrected(R1(i),C1(i),:);
end

featuremap2=[];
for i=1:3726
    featuremap2(i,:)=salinas_corrected(R2(i),C2(i),:);
end

featuremap3=[];
for i=1:1870
    featuremap3(i,:)=salinas_corrected(R3(i),C3(i),:);
end

featuremap4=[];
for i=1:1290
    featuremap4(i,:)=salinas_corrected(R4(i),C4(i),:);
end

featuremap5=[];
for i=1:2510
    featuremap5(i,:)=salinas_corrected(R5(i),C5(i),:);
end

featuremap6=[];
for i=1:3594
    featuremap6(i,:)=salinas_corrected(R6(i),C6(i),:);
end

featuremap7=[];
for i=1:3272
    featuremap7(i,:)=salinas_corrected(R7(i),C7(i),:);
end

featuremap8=[];
for i=1:10622
    featuremap8(i,:)=salinas_corrected(R8(i),C8(i),:);
end

featuremap9=[];
for i=1:5991
    featuremap9(i,:)=salinas_corrected(R9(i),C9(i),:);
end

featuremap10=[];
for i=1:2906
    featuremap10(i,:)=salinas_corrected(R10(i),C10(i),:);
end

featuremap11=[];
for i=1:934
    featuremap11(i,:)=salinas_corrected(R11(i),C11(i),:);
end

featuremap12=[];
for i=1:1690
    featuremap12(i,:)=salinas_corrected(R12(i),C12(i),:);
end

featuremap13=[];
for i=1:817
    featuremap13(i,:)=salinas_corrected(R13(i),C13(i),:);
end

featuremap14=[];
for i=1:949
    featuremap14(i,:)=salinas_corrected(R14(i),C14(i),:);
end

featuremap15=[];
for i=1:5835
    featuremap15(i,:)=salinas_corrected(R15(i),C15(i),:);
end

featuremap16=[];
for i=1:726
    featuremap16(i,:)=salinas_corrected(R16(i),C16(i),:);
end
...
opts.train.learningRate = 2.5*[ 0.01 * ones( 1,30 ) 0.005 * ones( 1, 30 ) 0.003 * ones( 1, 30 ) 0.001 * ones( 1,30 ) ];
f = 0.01;
...
imdb.images.data = zeros( 2 * HalfWidth+1 , 2 * HalfWidth +1, nLD, 48347, 'single' );
imdb.images.labels = zeros( 1, 48347 );

RandPerm_testing = randperm( 48347 );
for iSample = 1 : 48347
