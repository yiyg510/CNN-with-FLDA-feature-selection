...
function Class_Map = indian_pines_lda_wrapper(params) 

load Indian_pines_corrected.mat;
load Indian_pines_gt.mat;

disp( 'Data is imported...' );
GroundTruth=double(indian_pines_gt);
Indian_Pines=single(indian_pines_corrected);
clear Indian_pines_corrected;

[ nRow, nColumn, nBand ] = size( Indian_Pines );

HalfWidth=13;
Indian_Pines = 1 * ( ( Indian_Pines - min( Indian_Pines( : ) ) ) / ( max( Indian_Pines( : ) ) - min( Indian_Pines( : ) ) )-0.5);
...
[R14, C14] = find(GroundTruth==14);
[R15, C15] = find(GroundTruth==15);
[R16, C16] = find(GroundTruth==16);

C = [size(R1,1),size(R2,1),size(R3,1),size(R4,1),size(R5,1),size(R6,1),size(R7,1),size(R8,1),size(R9,1),size(R10,1),size(R11,1),size(R12,1),size(R13,1),size(R14,1),size(R15,1),size(R16,1)];

%%Defining each featuremap based on class
featuremap1=[];
for i=1:46 
    featuremap1(i,:)=indian_pines_corrected(R1(i),C1(i),:);
end

featuremap2=[];
for i=1:1348
    featuremap2(i,:)=indian_pines_corrected(R2(i),C2(i),:);
end

featuremap3=[];
for i=1:382
    featuremap3(i,:)=indian_pines_corrected(R3(i),C3(i),:);
end

featuremap4=[];
for i=1:105
    featuremap4(i,:)=indian_pines_corrected(R4(i),C4(i),:);
end

featuremap5=[];
for i=1:289
    featuremap5(i,:)=indian_pines_corrected(R5(i),C5(i),:);
end

featuremap6=[];
for i=1:730
    featuremap6(i,:)=indian_pines_corrected(R6(i),C6(i),:);
end

featuremap7=[];
for i=1:28
    featuremap7(i,:)=indian_pines_corrected(R7(i),C7(i),:);
end

featuremap8=[];
for i=1:280
    featuremap8(i,:)=indian_pines_corrected(R8(i),C8(i),:);
end

featuremap9=[];
for i=1:20
    featuremap9(i,:)=indian_pines_corrected(R9(i),C9(i),:);
end

featuremap10=[];
for i=1:825
    featuremap10(i,:)=indian_pines_corrected(R10(i),C10(i),:);
end

featuremap11=[];
for i=1:2182
    featuremap11(i,:)=indian_pines_corrected(R11(i),C11(i),:);
end

featuremap12=[];
for i=1:413
    featuremap12(i,:)=indian_pines_corrected(R12(i),C12(i),:);
end

featuremap13=[];
for i=1:205
    featuremap13(i,:)=indian_pines_corrected(R13(i),C13(i),:);
end

featuremap14=[];
for i=1:943
    featuremap14(i,:)=indian_pines_corrected(R14(i),C14(i),:);
end

featuremap15=[];
for i=1:99
    featuremap15(i,:)=indian_pines_corrected(R15(i),C15(i),:);
end

featuremap16=[];
for i=1:93
    featuremap16(i,:)=indian_pines_corrected(R16(i),C16(i),:);
end

featuremap = [featuremap1;featuremap2;featuremap3;featuremap4;featuremap5;featuremap6;featuremap7;featuremap8;featuremap9;featuremap10;featuremap11;featuremap12;featuremap13;featuremap14;featuremap15;featuremap16];
...
for i=1:C(14)
    FLDAmap(R14(i),C14(i),:)=LD(i+CSM(14),:);
end

for i=1:C(15)
    FLDAmap(R15(i),C15(i),:)=LD(i+CSM(15),:);
end

for i=1:C(16)
    FLDAmap(R16(i),C16(i),:)=LD(i+CSM(16),:);
end
...
result14 = find(correct_labels == 14);
result15 = find(correct_labels == 15);
result16 = find(correct_labels == 16);
...
randperm14 = randsample(result14, round(percentage * size([R14,C14],1)) );
randperm15 = randsample(result15, round(percentage * size([R15,C15],1)) );
randperm16 = randsample(result16, round(percentage * size([R16,C16],1)) );
buffer = ones(1,100);

RandPerm_training = [randperm1 randperm2 randperm3 randperm4 randperm5 randperm6 randperm7 randperm8 randperm9 randperm10 randperm11 randperm12 randperm13 randperm14 randperm15 randperm16 buffer];
...
nFeatureMapOfHSI( 3 ) = params.nFeatureMapOfHSI(1);
...
imdb.images.data = zeros( 2 * HalfWidth+1 , 2 * HalfWidth +1, nLD, 7988, 'single' );
imdb.images.labels = zeros( 1, 7988 );

RandPerm_testing = randperm(7988);
for iSample = 1 : 7988
