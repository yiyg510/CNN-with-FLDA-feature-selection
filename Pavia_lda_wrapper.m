...
function Class_Map = Pavia_cnn_wrapper(params) 

load Pavia.mat;
load Pavia_gt.mat;

disp( 'Data is imported...' );
GroundTruth=double(pavia_gt);
Pavia=single(pavia);

[ nRow, nColumn, nBand ] = size( Pavia );

HalfWidth=13;
Pavia = 1 * ( ( Pavia - min( Pavia( : ) ) ) / ( max( Pavia( : ) ) - min( Pavia( : ) ) )-0.5);
...
C = [size(R1,1),size(R2,1),size(R3,1),size(R4,1),size(R5,1),size(R6,1),size(R7,1),size(R8,1),size(R9,1)];

%%Defining each featuremap based on class
featuremap1=[];
for i=1:62625 
    featuremap1(i,:)=Pavia(R1(i),C1(i),:);
end

featuremap2=[];
for i=1:7108
    featuremap2(i,:)=Pavia(R2(i),C2(i),:);
end

featuremap3=[];
for i=1:2399
    featuremap3(i,:)=Pavia(R3(i),C3(i),:);
end

featuremap4=[];
for i=1:2685
    featuremap4(i,:)=Pavia(R4(i),C4(i),:);
end

featuremap5=[];
for i=1:6507
    featuremap5(i,:)=Pavia(R5(i),C5(i),:);
end

featuremap6=[];
for i=1:8905
    featuremap6(i,:)=Pavia(R6(i),C6(i),:);
end

featuremap7=[];
for i=1:7287
    featuremap7(i,:)=Pavia(R7(i),C7(i),:);
end

featuremap8=[];
for i=1:40596
    featuremap8(i,:)=Pavia(R8(i),C8(i),:);
end

featuremap9=[];
for i=1:2826
    featuremap9(i,:)=Pavia(R9(i),C9(i),:);
end

featuremap = [featuremap1;featuremap2;featuremap3;featuremap4;featuremap5;featuremap6;featuremap7;featuremap8;featuremap9];
...
RandPerm_training = [randperm1 randperm2 randperm3 randperm4 randperm5 randperm6 randperm7 randperm8 randperm9 buffer];
...
imdb.images.data = zeros( 2 * HalfWidth+1 , 2 * HalfWidth +1, nLD, 140938, 'single' );
imdb.images.labels = zeros( 1, 140938 );

RandPerm_testing = randperm( 140938 );
for iSample = 1 : 140938
